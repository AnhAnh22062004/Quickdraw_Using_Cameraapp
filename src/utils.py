...
def get_images(path, classes):
    images = [cv2.imread("{}/{}.png".format(path, item), cv2.IMREAD_UNCHANGED) for item in classes] # Read images and keep the alpha channel
    return images

def get_overlay(bg_image, fg_image, sizes=(40, 40)): # Overlay the foreground image on the background image
